/**
 * Plays out a match that was already created via the matchmaking queue
 * (/queue/join) — same negotiate/offer loop as run-match.ts, but skips
 * match creation since the queue already paired the players and assigned
 * a matchId.
 */
...
if (!matchId) throw new Error("usage: tsx scripts/play-queued-match.ts <matchId>");
...
async function getState(id: string, as: string) {
  const res = await fetch(`${WARDEN_URL}/match/${id}/state?as=${as}`);
  if (!res.ok) throw new Error(`getState failed: ${res.status} ${await res.text()}`);
  return res.json() as Promise<any>;
}

async function postMove(id: string, player: string, move: unknown) {
  const res = await fetch(`${WARDEN_URL}/match/${id}/move`, {
    method: "POST",
...
function buildHistory(state: any, me: string) {
  return state.rounds
    ...
    .map((r: any) => ({ round: r.index, myAsk: r.offers[me], oppAsk: r.offers[opponentOf(state, me)], myReceived: r.payoutFraction?.[me] }));
}

async function main() {
  console.log(`Playing queued match ${matchId}`);
  ...
  while (!settled) {
    ...
    for (const player of pending) {
      ...
      if (phase === "NEGOTIATE") {
        ...
        const moveResult = await postMove(matchId, player, { type: "offer", ask: decision.ask, escalate: decision.escalate });
        console.log(`[round ${r.index}] ${agent.name} offers ${decision.ask.toFixed(2)}${decision.escalate ? " (escalate)" : ""}`);
